...
boot_alloc(uint32_t n)
{
	...
	rounded_n = ROUNDUP(n, PGSIZE);
	if ((rounded_n / PGSIZE) <= npages_free) {
		npages_free -= rounded_n / PGSIZE;
		nextfree += rounded_n;
	} else {
...
mem_init(void)
{
    ...
    boot_map_region(kern_pgdir, UPAGES, ROUNDUP(sizeof(struct Page) * npages, PGSIZE),
            PADDR(pages), PTE_U | PTE_P);
    boot_map_region(kern_pgdir, (uintptr_t) pages, sizeof(struct Page) * npages,
...
page_init(void)
{
	...
	for (i = EXTPHYSMEM / PGSIZE; i < 0x400000 / PGSIZE; i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}

    for (i = 0x400000 / PGSIZE; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
    }
